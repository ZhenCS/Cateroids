let startScene = new Phaser.Scene(keys.STARTKEY);
...
startScene.create = function(){
    ...
    this.gameStart = this.createButton(gameHeight/2 + 150 * gameScale.scale, "Start Game");
    this.levelSelect = this.createButton(gameHeight/2 + 300 * gameScale.scale, "Level Select");
    this.controls = this.createButton(gameHeight/2 + 450 * gameScale.scale, "Controls");
    this.help = this.createButton(gameHeight/2 + 600 * gameScale.scale, "Help");

    this.initControls();

    this.gameStart.on('pointerdown', function() {
        this.scene.game.scene.switch(keys.STARTKEY, keys.GAMEKEY);
    });
    this.levelSelect.on('pointerdown', function() {
        this.scene.game.scene.switch(keys.STARTKEY, keys.LEVELSKEY);
    });
...
startScene.createButton = function(yPosition, text){
...
startScene.initControls = function() {
    let gameWidth = this.sys.game.config.width;
    let gameHeight = this.sys.game.config.height;

    this.controlContainer = this.add.sprite(gameWidth/2, gameHeight/2, keys.CONTROLS1KEY);
...
startScene.showControls = function() {
...
startScene.hideControls = function() {
    this.controlContainer.visible = false;
    this.backButton.visible = false;
