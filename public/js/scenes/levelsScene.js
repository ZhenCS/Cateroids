let levelsScene = new Phaser.Scene(keys.LEVELSKEY);

levelsScene.create = function(){
    setBG(this);
    setGameName(this);
    ...
    this.levelsContainer.setSize(rect.width - 300 * gameScale.scale, rect.height);
...
levelsScene.createLevelIcon = function(id, x, y){
    let iconContainer = this.add.container(x, y);
    let iconText = this.add.text(x - 25 * gameScale.scale, y - 45 * gameScale.scale, `${id}`, {
        font: `${90 * gameScale.scale}px impact`,
        fill: '#000000'
    });
    iconText.depth = 2;
    let iconBG = this.add.sprite(x, y, keys.LEVELICON).setInteractive({cursor: 'pointer'});
    ...
    for(var i = 0; i < 3; i++){
        let star = this.add.sprite(x - iconBG.displayWidth/2 + paddingStars + (100 * gameScale.scale * i), y + paddingStars + iconBG.displayHeight/2, keys.STARKEY);
        star.depth = 2;
