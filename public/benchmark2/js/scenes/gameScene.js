...
gameScene.create = function(){
    ...
    this.pauseButton = this.add.text(0, gameHeight - 100 * gameScale.scale, "ESC", {
...
gameScene.initPlayer1UI = function(){
    ...
    let {padding, barHeight, healthWidth, oxygenWidth, healthColor, oxygenColor, barColor} = gameStyles;
    
    this.health = this.add.graphics().fillStyle(healthColor).setDepth(gameConsts.uiDepth + 1);
    this.healthBG = this.add.graphics().fillStyle(barColor).setDepth(gameConsts.uiDepth);

    this.health.fillRect(padding, padding, healthWidth/2, barHeight);
    this.healthBG.fillRect(padding, padding, healthWidth, barHeight);
    

    this.oxygen = this.add.graphics().fillStyle(oxygenColor).setDepth(gameConsts.uiDepth + 1);
    this.oxygenBG = this.add.graphics().fillStyle(barColor).setDepth(gameConsts.uiDepth);

    this.oxygen.fillRect(padding, padding * 2 + barHeight, oxygenWidth/2, barHeight);
    this.oxygenBG.fillRect(padding, padding * 2 + barHeight, oxygenWidth, barHeight);
    

    player1Container.add([this.healthBG, this.health, this.oxygenBG, this.oxygen]);
...
gameScene.initPauseMenu = function(){
    ...
    let resumeButton = this.createButton(500 * gameScale.scale, "Resume Game").on('pointerdown', function(){
        this.scene.hidePauseMenu();
    });
    let controlButton = this.createButton(700 * gameScale.scale, "Controls").on('pointerdown', function(){
        this.scene.showControls();
    });
    let exitGameButton = this.createButton(900 * gameScale.scale, "Exit Game").on('pointerdown', function(){
    ...
    pauseContainer.add([pauseHeader, resumeButton, controlButton, exitGameButton]);
...
gameScene.initControls = function() {
    ...
    let controlContainer = this.add.sprite(gameWidth/2, gameHeight/2, keys.CONTROLS1KEY);
...
gameScene.createButton = function(yPosition, text){
    let button = this.add.text(0, yPosition, text, {
        font: `${100 * gameScale.scale}px impact`,
        fill: '#ffffff',
        stroke: 'black',
        strokeThickness: 5
